              ...
              <section>
                  ...
                  <div className="space-y-2">
                    {currentKeys.map((k, i) => (
                      <div key={i} className="flex items-center gap-2 bg-gray-800/60 border border-gray-700 rounded-lg px-3 py-2">
                        ...
                        <button onClick={() => removeProviderKey(aiProvider, i)} className="shrink-0 text-gray-600 hover:text-red-400 transition-colors" title="Remove key">
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    ))}
                ...
                {g && (
                  <div className={`flex items-start justify-between gap-3 ${needsKey ? 'mt-2' : ''}`}>
                    ...
                    {g.link && (
                      <a href={g.link} target="_blank" rel="noopener noreferrer" className="shrink-0 text-xs font-semibold text-indigo-400 hover:text-indigo-300 transition-colors whitespace-nowrap">
                        Get API Key →
                      </a>
                    )}
                  </div>
